...
class CartController extends Controller
{
    ...
    public function update(Request $request, int $packageId): JsonResponse
    {
        ...
        $success = $this->cartService->updateQuantity($packageId, $quantity);

        if (!$success) {
            return response()->json([
                'success' => false,
                'message' => 'Unable to update cart. Please check item availability.'
            ], 422);
        }
     ...
     */
    public function remove(int $packageId): JsonResponse
    {
        $success = $this->cartService->removeItem($packageId);
